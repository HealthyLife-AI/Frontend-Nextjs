...
export function AiSummaryCard({ summary }: { summary: AiSummary | null }) {
  ...
  return (
    <section className="rounded-panel border border-border bg-gradient-to-br from-primary/5 to-transparent p-5 shadow-panel">
      ...
      {/*
        `dir="rtl"` explicitly, not inherited: the summary is ALWAYS
        written in Arabic (see WeeklySummaryService's systemPrompt — it
        is generated by a scheduled job with no locale to follow, in the
        product's own language). On the English dashboard the page
        direction is LTR, and Arabic text in an LTR container renders
        with its alignment and trailing punctuation in the wrong place.
      */}
      <p dir="rtl" className="mt-3 text-start text-sm leading-relaxed text-ink">
